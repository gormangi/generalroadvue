      ...
      <div class="col-md-12">
        <div class="card">
          ...
          <div class="card-action">
            <button type="button" class="btn btn-success" @click="methods.childCategoryModify" style="margin-right:10px;">수정</button>
            <button type="button" class="btn btn-danger" @click="methods.childCategoryModifyCancel">취소</button>
          </div>
        </div>
      </div>
    </div>
...
const methods = {
  async modifyInit(categoryIdx) {
    startLoading();
    const data = await $fetch('/api/category/getCategoryInfo', {method: 'post', body: categoryIdx}).finally(() => stopLoading());
